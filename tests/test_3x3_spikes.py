...
def build_3x3_grid_spikes(dim=3, spike_factor=10.0, jitter=0.1, eps=1e-6, seed=42):
    """
    Build a 3x3 grid tensor network with diagonal spikes.
    
    Base entries are uniform around 1, with multiplicative spikes on the
    hyper-diagonal (where all indices are equal).
    
    Args:
        dim: Dimension of each tensor index
        spike_factor: Multiplicative factor for diagonal entries
    ...
    for i in range(grid_size):
        for j in range(grid_size):
            ...
            # Add multiplicative spikes on the hyper-diagonal
            idx_grid = np.indices(shape)
            mask = np.all(idx_grid == idx_grid[0], axis=0)
            T[mask] *= spike_factor
...
def test_trace_3x3_grid_spikes(dim=3,
                                A=200,
                                B=400,
                                C=200,
                                spike_factor=10.0,
    ...
    if show_diagnostics:
        print("\n" + "=" * 60)
        print("[summary] 3x3 Diagonal Spikes Grid Results")
        print("=" * 60)
